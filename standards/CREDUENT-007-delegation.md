...
**Author:** IDevSec  
**Date:** 2026-09-02  
**Related:** [CREDUENT-001](CREDUENT-001-agent-json.md), [CREDUENT-002](CREDUENT-002-attestation.md), [CREDUENT-006](CREDUENT-006-dynamic-attestation.md), [CREDUENT-008](CREDUENT-008-ebpf-guard.md)
...
### 3.3 Mandatory Proof-of-Possession (PoP) Key Binding

To eliminate token replay vulnerabilities (e.g., LiteLLM static credential theft attacks), CREDUENT-007 v0.3 mandates **Proof-of-Possession (PoP)** verification.

A CDT is cryptographically bound to the `delegate_public_key` specified in Section 3.1. Possessing the CDT token payload alone is **insufficient** to execute requests at an API Gateway / Policy Decision Point (PDP).

When presenting a CDT to a downstream service or API Gateway, the delegate MUST attach a short-lived request signature generated by its local private key matching `delegate_public_key`:

```http
X-CDT-Token: <Base64_Encoded_CDT_Payload>
X-CDT-PoP-Timestamp: 2026-09-02T04:26:00Z
X-CDT-PoP-Signature: <Ed25519_Signature_Over_HTTP_Method_Path_Timestamp>
```

#### PoP Signature Computation Formula:
$$\text{PoP\_Sig} = \text{Ed25519\_Sign}\left(\text{Delegate\_PrivateKey}, \text{HTTP\_Method} \mathbin{\Vert} \text{URI\_Path} \mathbin{\Vert} \text{Timestamp} \mathbin{\Vert} \text{CDT\_Token\_ID}\right)$$

If the `X-CDT-PoP-Signature` is missing, expired (> 60 seconds skew), or invalid, the receiving Gateway MUST reject the request with `401 Unauthorized` (`CDT_POP_SIGNATURE_REQUIRED`), regardless of whether the CDT token payload itself is valid.
