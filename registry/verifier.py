    ...
    try:
        ...
        for rdata in answers:
            ...
            if agent_id in txt_content:
                return True, ""
        return False, f"DNS TXT record at {txt_record_name} found, but does not contain agent_id '{agent_id}'."
...
def verify_agent_registration(agent_id: str, domain: str, agent_json_url: str) -> tuple[bool, str, dict]:
    """
    ...
    if doc.get("agent_id") != agent_id:
        return False, f"agent_id in agent.json ('{doc.get('agent_id')}') does not match registration agent_id ('{agent_id}').", {}
