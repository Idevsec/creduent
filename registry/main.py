...
def check_register_rate_limit(request: Request):
    ...
    if is_redis_configured():
        ...
        if not hasattr(app, "reg_rate_db"):
            app.reg_rate_db = {}
        app.reg_rate_db = {
            k: v for k, v in app.reg_rate_db.items() if v["expiry"] > now
        }
        if key in app.reg_rate_db:
            ...
            app.reg_rate_db[key] = {
                "count": 1,
                "expiry": now + REGISTER_RATE_LIMIT_WINDOW,
            }
...
def check_challenge_rate_limit(request: Request):
    ...
    if is_redis_configured():
        ...
        if not hasattr(app, "chal_rate_db"):
            app.chal_rate_db = {}
        app.chal_rate_db = {
            k: v for k, v in app.chal_rate_db.items() if v["expiry"] > now
        }
        if key in app.chal_rate_db:
            ...
            app.chal_rate_db[key] = {
                "count": 1,
                "expiry": now + CHALLENGE_RATE_LIMIT_WINDOW,
            }
...
def register(req: RegisterRequest, request: Request):
    ...
    success, reason, doc = verify_agent_registration(
        agent_id=req.agent_id, domain=req.domain, agent_json_url=req.agent_json_url
    )
    if not success:
...
def revoke(agent_id: str, request: Request):
    ...
    if not admin_key_env:
        raise HTTPException(
            status_code=500,
            detail="Admin revocation is not configured on server (CREDUENT_ADMIN_KEY env var missing).",
        )
...
def renew(req: RenewRequest, request: Request):
    ...
    if not pk_str.startswith("ed25519:"):
        raise HTTPException(
            status_code=400, detail="Unsupported public key format in attestation"
        )
    ...
    try:
        ...
        if not verified:
            raise HTTPException(
                status_code=400, detail="Invalid client signature for renewal"
            )
        ...
        raise HTTPException(
            status_code=400, detail=f"Signature verification error: {str(e)}"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to sign renewed attestation: {str(e)}"
        )
...
def register_webhook(req: WebhookRegisterRequest):
    ...
    try:
        ...
        # 1. Try JCS canonicalized dictionary
        payload_dict = {"agent_id": req.agent_id, "webhook_url": req.webhook_url}
        try:
            canonical_bytes = canonicalize(payload_dict).encode("utf-8")
            public_key.verify(signature_bytes, canonical_bytes)
            verified = True
        ...
        if not verified:
            raise HTTPException(
                status_code=403,
                detail="Invalid client signature for webhook registration",
            )
        ...
        raise HTTPException(
            status_code=403, detail=f"Signature verification error: {str(e)}"
        )
...
def attest(req: AttestRequest, request: Request):
    ...
    if not admin_key_env:
        raise HTTPException(
            status_code=500,
            detail="Admin attestation is not configured on server (CREDUENT_ADMIN_KEY env var missing).",
        )
    admin_key_env = admin_key_env.strip()
    creduent_admin_key = request.headers.get(
        "CREDUENT_ADMIN_KEY"
...
def verify_challenge(req: VerifyChallengeRequest, request: Request):
    ...
    if not pk_str.startswith("ed25519:"):
        raise HTTPException(
            status_code=400, detail="Unsupported agent public key format"
        )
    ...
    try:
        ...
        proof_obj = {**proof_payload, "signature": sig_b64}

        proof_token_str = base64.b64encode(
            json.dumps(proof_obj).encode("utf-8")
        ).decode("utf-8")
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to generate proof token: {str(e)}"
        )
