...
### Added
- **SDK Adapter Reversibility Guards**: Released runtime hotfixes across `creduent-python` and `creduent-js` adding entry-boundary `ProvenanceGuard` (fail-closed `IRREVERSIBLE` default on unclassified/un-bound tool metadata) and `LedgerChainVerifier` (independent Merkle ledger hash-chain step-count verifier).
