...
def verify_agent(target: str) -> dict:
    ...
    Args:
        target: The agent's agent:// URI (e.g. agent://idevsec/steward) or domain (e.g. creduent.idevsec.com).
    ...
    if response_obj["self_verified"]:
        ...
        registry_base = os.environ.get(
            "CREDUENT_REGISTRY_URL", "http://localhost:8001"
        ).rstrip("/")
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if verify_registry_signature(attestation_obj):
                    ...
                    response_obj["attestation_level"] = attestation_obj.get(
                        "level", "verified"
                    )
                    response_obj["attestation_issued_at"] = attestation_obj.get(
                        "issued_at"
                    )
                    response_obj["attestation_expires_at"] = attestation_obj.get(
                        "expires_at"
                    )
                else:
