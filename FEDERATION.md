...
The reference registry at `creduent.idevsec.com` publishes its own identity at `/.well-known/agent.json`. A federated trust chain can verify registries using the exact same mechanism used to verify agents.
...
|:---|:---|:---|
| Registry URL | Hardcoded to `creduent.idevsec.com` in SDKs | SDKs accept configurable `baseUrl` option (already supported) |
